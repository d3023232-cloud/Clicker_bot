...
async def handle_roulette_bet(update, context):
    ...
    text = "🎰 Ставка: " + format_number(bet) + " монет" + chr(10) + "Выберите цвет:"
...
async def handle_crash_bet(update, context):
    ...
    for m in multipliers:
        row.append(InlineKeyboardButton(str(m) + "x", callback_data='crash_multiplier_' + str(m)))
        if len(row) == 3:
            keyboard.append(row)
            row = []
    if row:
    ...
    text = "💥 Ставка: " + format_number(bet) + " монет" + chr(10) + "Выберите множитель:"
    await update.message.reply_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
...
async def handle_duel_bet(update, context):
    ...
    text = "⚔️ <b>Дуэль</b>" + chr(10) + result['message'] + chr(10) + chr(10) + "🪙 Баланс: " + format_number(int(result['balance']))
...
def signal_handler(sig, frame):
    ...
    print("🛑 Получен сигнал завершения. Сохраняем данные...")
